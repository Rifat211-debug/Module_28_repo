...
def predict_gpa(gender, age, address, famsize,
                ...
                relationships, smoker, tution_fee, time_friends, ssc_result):
    #Pack Inputs into a Dataframe
    input_df = pd.DataFrame([[gender, age, address, famsize,
                Pstatus, M_Edu, F_Edu, M_Job, F_Job,
                relationships, smoker, tution_fee, time_friends, ssc_result]],
                columns = [
                    'gender', 'age', 'address', 'famsize','Pstatus', 'M_Edu', 'F_Edu', 'M_Job', 'F_Job', 'relationships', 'smoker', 'tution_fee', 'time_friends','ssc_result'
                    ]
                ) 
...
inputs = [
    ...
    gr.Slider(0, 4, steps = 1, label = "Mother's Edu"),
    gr.Slider(0, 4, steps = 1, label = "Father's Edu"),
    ...
    gr.Number(label = "Tution Fee"),
    gr.Slider(1, 5, steps = 1, label = "Time with Friends"),
